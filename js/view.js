...
view.editDocument = function (e) {
    ...
    <div class="form-row" style="padding-top: 1rem;">
        ...
        <div class="col-sm-6 mb-3">
            <input type="text" class="form-control" name="university" value="${documentNeedEdit.university}"
                placeholder="Trường">
        </div>
    </div>
    <div class="form-row">
    ...
    <div class="form-row">
        ...
        <div class="col-sm-6 mb-3">
            <textarea class="form-control" rows="3" name="describe" value="${documentNeedEdit.describe}"
                placeholder="Mô tả"></textarea>
        </div>
    </div>
    <img style="width: 100px; height: 100px;" src="${documentNeedEdit.image}" alt="${documentNeedEdit.name}" />
</div>
<button class="btn btn-primary" type="submit">Đăng</button>
<button class="btn btn-danger" id="main-link">Hủy</button>`;

    $("#form-fix-document").submit(function (event) {
        event.preventDefault();

        let document = {
            id: documentNeedEdit.id,
            CreateAt: documentNeedEdit.CreateAt,
            owner: documentNeedEdit.owner,
            sellStatus: documentNeedEdit.owner,
            name: formFixDocument.sellStatus,
            university: formFixDocument.university.value,
            condition: formFixDocument.condition.value,
            // image: [await controller.getImage('form-fix-document')],
        ...
        for (let j = i * keyNumber; j < i * keyNumber + keyNumber; j++)
            if (j < end) {
                ...
                s += `<div class="col-md-6 col-lg-4 col-xl-3 card--padding-right">
                    <div class="card card--hover" id="${element.id}" onclick = "view.moveToProductDetails(this)">
                        <img src="${element.image}"
                        class="card-img-top" alt="..." style="height: 200px; width: cover">
                        <div class="card-body">
    ...
    if (!model.currentUser) {
        ...
        if (model.currentUser.email == emailOfOwner) {
            ...
            if (first == 'true') {
                ...
                alert('Bạn đã có cuộc hội thoại với người này!');
                view.showComponents('main')
            }
        }
    }
        ...
        for (let conversation of listconversation) {
            ...
            if (model.currentUser) {
                let className = (model.currentConversation && model.currentConversation.id == conversation.id)
                    ...
                    : 'chat-element';
                if (firebase.auth().currentUser.email == conversation.title.emailOwner) {
                            ...
                            <div class="col-6">
                                <ul class="nav navbar-nav navbar-right ml-auto">
                                    <li class="nav-item dropdown">
                                        <a style="color: rgb(206, 212, 218)" data-toggle="dropdown" class="nav-link dropdown-toggle user-action">
...
view.showPagination = function (listDocument) {
...
view.showContentLeftBySearch = async function (id) {
    ...
    if (model.currentProductBySearch) {
        listDocument = model.currentProductBySearch;
    }
    let classNameOfAll = (model.currentUniversity == 'allUniversity')
        ? 'university current'
        : 'university';
    ...
    let uniqueSet = new Set(listUniversity);
    listUniversityClean = [...uniqueSet];
    console.log(listUniversityClean)
    for (let i = 0; i < listUniversityClean.length; i++) {
        let number = 0;
        for (let j = 0; j < listUniversity.length; j++) {
            if (listUniversityClean[i] == listUniversity[j]) {
...
view.showListDocumentCompare = async function (id, listDocumentCompare) {
    ...
    <thead>
       <tr>
        ...
        <ul class="nav navbar-nav navbar-right ml-auto">
          <li class="nav-item dropdown">
              <a style='color:white' href="#" data-toggle="dropdown" class="nav-link dropdown-toggle user-action">Giá       
              </a>
              <ul class="dropdown-menu">
                  <li><a href="#" id="dropdown-profile-btn" class="dropdown-item"><i class="fa fa-user-o"></i>
                          Tăng dần</a></li>
                  <li><a href="#" id="dropdown-for-sale-btn" class="dropdown-item"><i class="fa fa-calendar-o"></i>
                          Giảm dần</a></li>
                </ul>
            </li>
        </ul>
           </th>
        <th scope="col"><ul class="nav navbar-nav navbar-right ml-auto">
        <li class="nav-item dropdown">
            <a style='color:white' href="#" data-toggle="dropdown" class="nav-link dropdown-toggle user-action">Giá       
            </a>
            <ul class="dropdown-menu">
